...
export default function PlayerProfilePage({ params }: { params: { slug: string } }) {
  ...
  const [characters, setCharacters] = useState<CharacterUsage[]>([]);
  ...
  useEffect(() => {
    async function fetchProfile() {
      try {
        const res = await fetch(`/api/players/${params.slug}`);
        ...
        if (data.success) {
          ...
          setError(data.error || 'Fighter details could not be found.');
        }
      } catch (err: any) {
        setError(err.message || 'An error occurred loading the fighter profile.');
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="space-y-8 animate-pulse">
        <div className="h-48 rounded bg-[#111] border border-[#2A2A2A] shimmer" />
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="h-64 rounded bg-[#111] border border-[#2A2A2A] shimmer" />
          </div>
  ...
  if (error || !player) {
    return (
      <div className="py-20 text-center" id="not-found-state">
        <div className="text-6xl mb-6">🥊</div>
        <h2 className="text-3xl tekken-heading text-[#C8102E] font-black mb-2">Fighter Not Found</h2>
        <p className="text-gray-400 max-w-md mx-auto mb-8">
          {error || "We couldn't track down this combatant. They might be in another bracket."}
        </p>
        <Link
          href="/"
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Main Layout Split */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Left Columns: Tournament History & Recent Matches */}
